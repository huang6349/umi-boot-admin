...
import Dropdown from './OperatorDropdown';
...
class OperationGroup extends React.Component {
  renderOperating = () => {
    ...
    if (Array.isArray(renders) && renders.length > 0) {
      ...
      if (arrAfter.length > 0) {
        ...
        arrAfterRender = <Dropdown ctx={ctx} config={{ render: finalAfterRender }} key={ctx.record.id} />;
      }
  ...
  render() {
    ...
    return (
      <ASpace size={spaceSize} style={props.style || {}}>
